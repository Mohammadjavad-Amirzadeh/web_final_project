...
def initiate_reservation():
    ...
    if start_time < datetime.now():
        return jsonify({'error': 'Reservation start time must be in the future'}), 400
    ...
    if duration < min_duration or duration > max_duration:
        return jsonify({
            'error': 'Reservation duration must be 60 minutes'
        }), 400
...
def confirm_half_payment(pending_id):
    ...
    reservation.half_paid_at = datetime.now()
...
def confirm_full_payment(id):
    ...
    reservation.fully_paid_at = datetime.now()
    reservation.confirmed_at = datetime.now()
...
@reservation_bp.route('/<int:user_id>/unpaid')
def unpaid_reservations_of_user(user_id):
    payload, error_response, status_code = verify_jwt_request(request=request)
    if error_response:
        return error_response, status_code
    ...
    if requesting_user_id != user_id and not getattr(requesting_user, 'is_admin', False):
        return jsonify({"error": "Permission denied. Only admins or the user themselves can access this."}), 403

    reservations = Reservation.query.filter_by(user_id=user_id, is_paid=False).all()
    if not reservations:
        return jsonify({'error': 'No unpaid reservations found for this user'}), 404
...
@reservation_bp.route('/<int:user_id>/paid')
def paid_reservations_of_user(user_id):
    payload, error_response, status_code = verify_jwt_request(request=request)
    if error_response:
        return error_response, status_code
    ...
    reservations = Reservation.query.filter_by(user_id=user_id, is_paid=True).all()
    if not reservations:
        return jsonify({'error': 'No paid reservations found for this user'}), 404
...
@reservation_bp.route('/free-mode', methods=['POST'])
def reserve_device_free_mode():
    payload, error_response, status_code = verify_jwt_request(request=request)
    if error_response:
        return error_response, status_code
    ...
    if not device_id_raw or not start_time_str:
        return jsonify({'error': 'Missing required fields'}), 400
    ...
    end_time = next_reservation.start_time if next_reservation else start_time + timedelta(days=2)
    ...
    for _ in range(max_retries):
        try:
            ...
            return jsonify({
                'message': f'Free mode reservation successful. It will expire at {end_time.isoformat()}.',
                'reservation_id': reservation.id,
                'pending_id': reservation.pending_id
            }), 201
    ...
    return jsonify({'error': 'Could not create a unique reservation. Please try again.'}), 500
